...
export const GET: APIRoute = ({ site }) => {
  ...
  const paths = ['/', ...Object.values(categoryPaths), ...Object.values(vtuberSections).map((section) => section.path), ...(toolsData as Tool[]).filter((tool) => tool.enabled).map((tool) => `${tool.path}/`)];
